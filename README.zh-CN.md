...
- **基因本体图谱** — 基于 React Flow 的交互式知识图谱，支持浏览与编辑
...
基于 React Flow 的交互式知识图谱，支持节点搜索、编辑和关系浏览。
...
- 对话历史保存在浏览器 `localStorage`，清除浏览器缓存后会丢失
- 富集分析使用本地 MH63 水稻注释文件，仅支持水稻基因 ID（`OsMH_` 前缀）
- 首次启动时注释文件加载需要约 10–30 秒
- 默认 LLM 模型为 `qwen-turbo`，可在设置页切换
